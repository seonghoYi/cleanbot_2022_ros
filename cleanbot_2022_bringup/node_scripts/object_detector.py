...
from cleanbot_2022_msgs.msg import BoundingBox
import cv2
import numpy as np
...
def main():
    ...
    rospy.init_node("cleanbot_2022_object_detector", anonymous=True)
    ...
    image_pub = rospy.Publisher("/cleanbot_2022/detected_image", rosImage, queue_size=10)
    bounding_box_pub = rospy.Publisher("/cleanbot_2022/bounding_boxes", BoundingBoxes, queue_size=10)

    print("object_detector: model loading...")
    labels = read_label_file(model_label)
    interpreter = make_interpreter(model_file)
    interpreter.allocate_tensors()
    print("object_detector: model was loaded.")
    print("object_detector: detector running.")
